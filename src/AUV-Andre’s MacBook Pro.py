...
from imc_ros_interface.msg import Chlorophyll, Temperature, Salinity, EstimatedState, Sms
...
class AUV:
    ...
    __currentSalinity = .0
    __currentTemperature = .0
    __currentChlorophyll = .0
    __vehicle_pos = [0, 0, 0]
    ...
    def TemperatureCB(self, msg):
        self.__currentTemperature = msg.value.data
    
    def ChlorophyllCB(self, msg):
        self.__currentChlorophyll = msg.value.data
    ...
    def send_SMS_mission_complete(self):
        ...
        SMS.timeout.data = 60
        pos = self.__vehicle_pos
        SMS.contents.data = f"Congrats, Mission complete! LAUV-THOR position lat {pos[0]}, lon {pos[1]}"
        self.sms_pub_.publish(SMS)
        print("Finished SMS sending!")

    def send_SMS(self, message):
        print("[ACTION] [AUV] Sending SMS")
        SMS = Sms()
        SMS.number.data = self.__phone_number
        SMS.timeout.data = 30
        SMS.contents.data = message
        try:
            self.sms_pub_.publish(SMS)
            print("[INFO] [AUV] finish sending SMS")
        except:
            print("[ERROR] [AUV] failed to send SMS")
        

    def send_SMS_AUV_pos(self):
        print("[ACTION] [AUV] Sending SMS with vehicle pos")
        SMS = Sms()
        SMS.number.data = self.__phone_number
        SMS.timeout.data = 60
        pos = self.__vehicle_pos
        SMS.contents.data = f"LAUV-THOR position lat {pos[0]}, lon {pos[1]}"
        self.sms_pub_.publish(SMS)
        print("[INFO] [AUV] finish sending SMS")
    ...
    def get_chlorophyll(self):
        return self.__currentChlorophyll
    ...
    def set_submerged_time(self, time):
        self.__max_submerged_time = time

    def set_popup_time(self, time):
        self.__min_popup_time = time
