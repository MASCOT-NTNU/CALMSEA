...
class Boundary:
    ...
    def __init__(self,
        ...
        if border_file is not None:
            self.border_file = os.getcwd() + border_file
    ...
    def create_box_xy_border_file_from_loc(loc: np.ndarray, size:float, file_name: str, loc_type: str):
        ...
        # Save the box
        file = f"src/csv/{file_name}_xy.csv"
...
if __name__ == "__main__":
    ...
    files = [("/src/csv/simulation_border_xy.csv", "xy", "box_xy"),
             (f"/src/csv/{test_name1}_xy.csv", "xy", "box_test_xy"),
            (f"/src/csv/{test_name2}_xy.csv", "xy", "box_test_latlon"),
            ("/src/csv/polygon_border.csv", "latlon", "box_latlon"),
            ("/src/csv/simulation_border_latlon.csv", "latlon", "torndheim_latlon"),
            (None, None, "none"),
            ("/src/csv/arrow_border_xy.csv", "xy", "arrow_xy"),
            ("/src/csv/arrow_border_latlon.csv", "latlon", "arrow_latlon"),
            ("/src/csv/mausund_mission_area.csv", "latlon", "mausund_latlon")]
