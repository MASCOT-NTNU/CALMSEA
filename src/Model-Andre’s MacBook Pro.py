...
class Model:
    ...
    def filter_surface_points(self, S, T, Y):
        # This function removes the points at the surface
        # This is done to avoid the surface points to dominate the model
        # The surface points are not very informative
        # The surface points are removed if the depth is less than 0.5 meters
        depth = S[:,2]
        n_old = len(Y)
        indecies = np.where(depth > 0.5)
        S = S[indecies]
        T = T[indecies]
        Y = Y[indecies]
        n_new = len(Y)
        if self.print_while_running:
            print(time_now_str(), f"[INFO] [MODEL] Removed {n_old - n_new} points at the surface")
        return S, T, Y
    ...
    def reduce_points(self, S, T, Y):
        ...
        # Downfilter the points 
        S_new = np.empty((n_new, 3))
        T_new = np.empty(n_new)
        Y_new = np.empty(n_new)

        for i in range(n_new):
            indecies = np.array([i * reduce_factor + j for j in range(reduce_factor)])
            for j in indecies:
                if j >= n:
                    indecies = np.arange(i * reduce_factor, n)

            S_new[i] = np.mean(S[indecies], axis=0)
            T_new[i] = np.mean(T[indecies])
            Y_new[i] = int(np.mean(Y[indecies]))

        if self.print_while_running:
            print(time_now_str(), f"[INFO] [MODEL] Reduced {n} points to {n_new} points")

        return S_new, T_new, Y_new
    
    def filter_points(self, S, T, Y):

        # Filter the surface points
        S, T, Y = self.filter_surface_points(S, T, Y)

        # Reduce the number of points
        S, T, Y = self.reduce_points(S, T, Y)
    ...
    def add_new_values(self, S_new: np.ndarray, T_new: np.array, Y_new: np.array):
        ...
        # Filter the points
        S_new, T_new, Y_new = self.filter_points(S_new, T_new, Y_new)
    ...
    def down_sample_points(self):
        ...
        if self.data_dict["has_data"] == False:
            print(time_now_str(), "[WARNING] [MODEL] model has no points to downsample")
            return
    ...
    def get_t_now(self):
        ...
        if self.data_dict["has_data"] == False:
            print(time_now_str(), "[WARNING] [MODEL] no points in model")
            return time.time()
    
        if len(self.data_dict["T"]) > 0:
