...
class ModelLogNormal(Model):
    ...
    def filter_Y_zeros(self, S: np.ndarray, T: np.ndarray, Y: np.ndarray):
        # Filter the observations where Y is zero
        ind = np.where(Y > 0)
        len_o = len(Y)
        S, T, Y = S[ind], T[ind], Y[ind]
        len_n = len(Y)

        if self.print_while_running:
            print(time_now_str(), f"Surface points reduced from {len_o} too {len_n}")
        return S, T, Y
    
    def filter_points(self, S: np.ndarray, T: np.ndarray, Y: np.ndarray):
        # Filter the points where the observations are zero
        S, T, Y = self.filter_Y_zeros(S, T, Y)
