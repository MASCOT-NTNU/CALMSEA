...
class PathPlanner:
    ...
    def join_consecutive_paths(self, paths):
        ...
        waypoints = np.array(paths[0]["waypoints"])
        S = paths[0]["S"]
        T = paths[0]["T"]
        for path in paths:
    ...
    def get_possible_paths_z(self, start, end, path_depth_limits, t_start=0, min_depth_step=1):
        ...
        all_depth_limits = [start_depth_limits, target_depth_limits, path_depth_limits, self.auv_depth_range]
    ...
    def join_depth_ranges(self, depth_ranges):
        ...
        return [np.max(depth_ranges[:,0]), np.min(depth_ranges[:,1])]
